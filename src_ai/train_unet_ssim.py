...
class CombinedLoss(nn.Module):
    def __init__(self, alpha=0.5):
        ...
        self.alpha = alpha
        self.mse = nn.MSELoss()
        # data_range is 1.0 because our images are normalized between 0 and 1
        self.ssim = SSIM(data_range=1.0, size_average=True, channel=1)

    def forward(self, prediction, target):
        loss_mse = self.mse(prediction, target)
        # SSIM returns 1 for perfect match, so we minimize (1 - SSIM)
        loss_ssim = 1 - self.ssim(prediction, target) 
...
class SirtDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # Per-image Normalization (Min-Max)
...
def train():
    ...
    plt.xlabel('Epoch')
    plt.ylabel('Loss (SSIM + MSE)')
    plt.title('Training and Validation Loss')
    plt.legend()
    plt.show()

    torch.save(model.state_dict(), "sirt_unet_model_ssim.pth")
    print("Model saved to sirt_unet_model_ssim.pth")
